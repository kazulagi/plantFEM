...
type(IO_) :: f
integer(int32),allocatable :: domainIDs1(:),domainIDs2(:),domainIDs21(:),domainIDs12(:),InterConnect(:)
...
do NodeID=1, domain2%nn()
    ...
    ! get shape-function
    !sf = domain2%getShapeFunction(position=position)
...
!print *, "Interface: started."
!! create Elemental Matrices and Vectors
!allocate(DomainIDs21(domain1%nne()+1 ) )
...
!
!    ! set global coordinate
!    position(:) = domain2%mesh%nodcoord(NodeID,:)
!
!
!    ! Get the nearest Element to (x,y,z) = (0,0,1)
!    !print *, "domain1, ELEMENT ID: ", domain%mesh%nearestElementID(x=position(1),y=position(2),z=position(3) )
...
!    
!    ! get shape-function
!    !sf = domain1%getShapeFunction(position=position)
!    InterConnect(1) = NodeID
!    if( domain1%mesh%nearestElementID(x=position(1),y=position(2),z=position(3))<=0 )then
!        cycle
!    endif
!
!    InterConnect(2:) = domain1%connectivity(domain1%mesh%nearestElementID(x=position(1),y=position(2),z=position(3) ))
!    A_ij = penalty*domain1%connectMatrix(position,DOF=domain1%nd() ) 
!
!    ! assemble them 
!    call solver%assemble(&
!        connectivity=InterConnect,&
