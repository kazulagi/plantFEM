...
call cube%create("Cube3D",x_num=100,y_num=4,z_num=4)
call cube%resize(x=40.0d0,y=2.0d0,z=2.0d0)
...
v = zeros(6)
E_G(1) = 100.0d0*10**4
E_G(2) = 10000.0d0*10**4
E_G(3) = 10000.0d0*10**4
E_G(4) = 7.69d0*10**3
E_G(5) = 7.69d0*10**3
E_G(6) = 7.69d0*10**3
v(:) = 0.30d0
rot_angles = radian([0.0d0,90.0d0,0.0d0])

pressure = 200*1000.0d0*ones(cube%ne())

!$OMP parallel 
!$OMP do
do i = 1, cube%ne()
    call solver%setMatrix(DomainID=1,ElementID=i,DOF=3,&
       Matrix=cube%StiffnessMatrix(ElementID=i,E=E_G, v=v,rot_angles=rot_angles))
    call solver%setVector(DomainID=1,ElementID=i,DOF=3, &
        Vector=cube%PressureVector(&
                ElementID=i,&
                Pressure=pressure(i) &
            )&
    )
