program main
    ...
    print *, "Export results for >> ",trim(mpid%Name)
    ! For MPI-para
    ...
    val=5.0d0**(dble(mpid%Myrank))/10000.0d0
    print *, "Permiability :: ", val
    !call Permiability%create(Name="Permiability",ParaValue=10.0d0,Layer=1)
    call Permiability%create(Name="Permiability",ParaValue=val,Layer=1)
    ...
    ! visualize material
    call YoungsModulus%gmsh(Name=trim(mpid%Name)//"YoungsModulus",Tag="M : YoungsModulus (kPa)")
    call PoissonRatio%gmsh(Name=trim(mpid%Name)//"PoissonRatio",Tag="M : PoissonRatio")
    call Permiability%gmsh(Name=trim(mpid%Name)//"Permiability",Tag="M : Permiability (cm/s)")
    
    call    a_Psi%gmsh(Name=trim(mpid%Name)//"a_Psi   ", Tag="a_Psi   ")  
    call      a_P%gmsh(Name=trim(mpid%Name)//"a_P     ", Tag="a_P     ")
    call theta_eq%gmsh(Name=trim(mpid%Name)//"theta_eq", Tag="theta_eq")     
    call   Psi_eq%gmsh(Name=trim(mpid%Name)//"Psi_eq  ", Tag="Psi_eq  ")   
    call      a_E%gmsh(Name=trim(mpid%Name)//"a_E     ", Tag="a_E     ")
    call      a_v%gmsh(Name=trim(mpid%Name)//"a_v     ", Tag="a_v     ")
    call     E_eq%gmsh(Name=trim(mpid%Name)//"E_eq    ", Tag="E_eq    ") 
    call     v_eq%gmsh(Name=trim(mpid%Name)//"v_eq    ", Tag="v_eq    ") 
    ...
    ! visualize Domains
    call tissue%gmsh(Name=trim(mpid%Name)//"tissue",Tag="Tissue Domain")
    call water%gmsh(Name=trim(mpid%Name)//"water",Tag="Water Domain")
    
    call tissue%export(Name=trim(mpid%Name)//"tissue")
    call water%export(Name=trim(mpid%Name)//"water")
