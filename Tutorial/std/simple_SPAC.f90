program main
    ...
    enddo
    call f%close()
    call f%plot(option="with lines;set datafile separator ','; replot ")
    ! >>>>>>>> H/V spectra >>>>>>>>
    ...
    enddo
    call f%close()
    call f%plot(option="with lines;set datafile separator ',';&
         set xr[:"+str(sampling_Hz/5)+"];replot;"+&
        "; replot")
    call f%plot(option="with lines;&
        set datafile separator ','; replot;&
        set xr[:"+str(sampling_Hz/5)+"];replot;"+&
        "set terminal png; set output '"+&
        filepath+"_Rayl-Dispersion.png';replot; exit;")
    ! >>>>>>>> Rayleigh wave dispersion curve >>>>>>>> 
