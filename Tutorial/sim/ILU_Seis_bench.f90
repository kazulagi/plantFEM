...
program main
    ...
    siml%modal%solver%er0  = dble(1.0e-7)
    siml%modal%solver%relative_er  = dble(1.0e-7)
    ! 1 kHz sampling
    dt = 1.0d0/1000.0d0
    ...
    do i_i=1,200000
        ...
        call siml%solve(dt=dt,timeIntegral="Nemwark-beta",use_same_matrix=.true.,preconditioning="ILU")
        
        v = reshape(siml%v,domains(1)%nn(),domains(1)%nd() )
        !if(mod(i_i-1,2)==0 )then
        !call domains(1)%vtk("v_"+"x_step_"+zfill(i_i,5), v(:,1) )
        !call domains(1)%vtk("v_"+"y_step_"+zfill(i_i,5), v(:,2) )
        !call domains(1)%vtk("v_"+"z_step_"+zfill(i_i,5), v(:,3) )
        !endif
        do j_j=1,37
