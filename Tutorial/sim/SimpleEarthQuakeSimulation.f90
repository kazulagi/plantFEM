...
program main
    ...
    YoungModulus(:) = (Vs(1)*Vs(1)*density*density)*2.0d0*(1.0d0+0.300d0)
    YoungModulus(domains(1)%getElementList(zmax=-20.0d0) ) = (Vs(2)*Vs(2)*density*density)*2.0d0*(1.0d0+0.300d0)
    
    call domains(1)%rotate(x=radian(30.0d0) )
    YoungModulus(domains(1)%getElementList(zmax=0.0d0) ) = (Vs(3)*Vs(3)*density*density)*2.0d0*(1.0d0+0.300d0)
    call domains(1)%rotate(x=radian(-30.0d0) )
    
    YoungModulus(domains(1)%getElementList(zmax=-100.0d0) ) = (Vs(3)*Vs(3)*density*density)*2.0d0*(1.0d0+0.300d0)
    ...
    do i_i=1,10000
        ...
        if(1 <= i_i .and. i_i<=10)then
            call sim%setBoundary(DomainID=1,NodeList=domains(1)%getNodeList(&
                xmin=450.0d0,xmax=550.0d0,&
                ymin=450.0d0,ymax=550.0d0,&
                zmin=-280.0d0,zmax=-240.0d0)&
                ,condition="A",boundaryValue=[0.10d0,0.0d0,0.030d0])
        ...
        if(mod(i_i,2)==0 )then
            ...
            call domains(1)%vtk("x_step_"+zfill(i_i/2,4), v(:,1) )
            call domains(1)%vtk("y_step_"+zfill(i_i/2,4), v(:,2) )
            call domains(1)%vtk("z_step_"+zfill(i_i/2,4), v(:,3) )
        endif
