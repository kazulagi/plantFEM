program main
    ...
    ! initalize reactor
    call reactor%init()
    call reactor%put("CO", CO)
    call reactor%put("O2"  , O2)
    call reactor%put("CO2" ,  CO2)
    ! define reactor with reaction coefficient (in here, = 1.0)
    call reactor%define([str("CO"),str("O2")],[str("CO2")],0.10d0,&
      mol_rate=[2.0d0,1.0d0,1.0d0],reaction_order=[1.80d0,1.0d0])
    ...
    do
      ...
      call console%log("How much CO (mol/L) do you add?>>>")
      call console%read() 
      CO = CO + console%asReal()
