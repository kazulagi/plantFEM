...
program main
    ...
    do i_i=1,10000
        ...
        !    LinearSolver=BiCGSTAB_CRS_ACC_FP32)
        call sim%solve(dt=dt,timeIntegral="Nemwark-beta",use_same_matrix=.true.)
        v = reshape(sim%v,domains(1)%nn(),domains(1)%nd() )
        
        if( mod(i_i,100)==0)then
            call domains(1)%vtk("x_step_"+zfill(i_i/100,4), v(:,1) )
            call domains(1)%vtk("y_step_"+zfill(i_i/100,4), v(:,2) )
            call domains(1)%vtk("z_step_"+zfill(i_i/100,4), v(:,3) )
        endif
