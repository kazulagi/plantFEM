module wheatClass
    ...
    type :: wheat_NodeID_Branch_
        integer(int32),allocatable :: ID(:)
    contains 
        !procedure, public :: sync => syncwheat_NodeID_Branch
    end type
    ...
    type :: wheat_
        ...
        ! for panicle
        integer(int32) :: wheat_panicle_seed_division(1:3) = [3,3,3]
        ...
        type(wheat_NodeID_Branch_),allocatable :: NodeID_Branch(:)
        ...
        type(wheat_internode_info_),allocatable :: InterNodeInfo(:)
    ...
    contains
        procedure,public :: init => createwheat
        procedure,public :: create => createwheat
        procedure,public :: msh => mshwheat
        procedure,public :: vtk => vtkwheat
        procedure,public :: stl => stlwheat
        procedure,public :: json => jsonwheat
        procedure,public :: update => updatewheat
        
        ! Editor
        procedure,public :: remove => removewheat
        procedure,public :: rotate => rotatewheat
        procedure,public :: move => movewheat

        procedure,public :: getElementList => getElementListwheat
        ! Info
        !procedure,public :: properties => propertieswheat
        ! number of subdomain
        procedure,public :: ns => nswheat
        ! number of element
        procedure,public :: ne => newheat
        ! number of points
        procedure,public :: nn => nnwheat
        ! MemSize
        procedure,public :: checkMemoryRequirement => checkMemoryRequirementwheat
        ! number of stem, leaf ...etc.
        procedure,public :: numleaf => numleafwheat
        procedure,public :: numstem => numstemwheat
        procedure,public :: numroot => numrootwheat
        procedure,public :: numPanicle => numPaniclewheat
        ! get pointers
        procedure,public :: getFEMDomainPointers => getFEMDomainPointerswheat
        ! check data
        procedure,public :: checkYoungModulus => checkYoungModuluswheat
        procedure,public :: checkPoissonRatio => checkPoissonRatiowheat
        procedure,public :: checkDensity => checkDensitywheat
        ! get data
        procedure,public :: getYoungModulus => getYoungModuluswheat 
        procedure,public :: getPoissonRatio => getPoissonRatiowheat
        procedure,public :: getDensity => getDensitywheat
        
        procedure,public :: getYoungModulusField => getYoungModulusFieldwheat
        procedure,public :: getPoissonRatioField => getPoissonRatioFieldwheat
        procedure,public :: getDensityField => getDensityFieldwheat
        procedure,public :: getStressField => getStressFieldwheat
        
        ! alternative setters
        procedure,public :: setYoungModulus => setYoungModuluswheat
        procedure,public :: setPoissonRatio => setPoissonRatiowheat
        procedure,public :: setDensity => setDensitywheat

        ! simulator
        procedure,public :: getEigenMode => getEigenModewheat
        procedure,public :: getDisplacement => getDisplacementwheat
        procedure,public :: deform => deformwheat

        ! export eigen modes and frequency
        procedure,public :: export_eig => export_eigwheat
        
    end type
contains

! #############################################################
subroutine createwheat(this,config,debug)
    class(wheat_),intent(inout) :: this
    character(*),intent(in) :: config
    character(:),allocatable :: line
    logical,optional,intent(in) :: debug
    logical :: debug_log
    type(IO_) :: wheatconfig
    type(Random_) :: random
    ...
    if(debug_log)then
        cpid = cpid + 1
        call print("createwheat #" + str(cpid) )
    endif
    this%mainstem_length = freal(wheatconfig%parse(config,key1="Mainstem",key2="Length"))
    this%mainstem_width = freal(wheatconfig%parse(config,key1="Mainstem",key2="Width"))
    this%mainstem_node = fint(wheatconfig%parse(config,key1="Mainstem",key2="Node"))
    this%ms_angle_ave = freal(wheatconfig%parse(config,key1="Mainstem",key2="ms_angle_ave"))
    this%ms_angle_sig = freal(wheatconfig%parse(config,key1="Mainstem",key2="ms_angle_sig"))
    ...
    do
        ...
        line = wheatconfig%parse(config,key1="Leaf_"//str(this%num_leaf)//"_",key2="From")
    ...
    do i=1,this%num_leaf
        this%leaf_From(i)= fint(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="From"))
        !this%leaf_Length(i)= freal(wheatconfig%parse(&
        !    config,key1="Leaf_"//str(i)//"_",key2="Length"))
        !this%leaf_Width(i)= freal(wheatconfig%parse(&
        !    config,key1="Leaf_"//str(i)//"_",key2="Width"))
        this%leaf_curvature(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_curvature"))
        this%leaf_thickness_ave(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_thickness_ave"))
        this%leaf_thickness_sig(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_thickness_sig"))
        this%leaf_angle_ave_x(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_angle_ave_x"))
        this%leaf_angle_sig_x(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_angle_sig_x"))
        this%leaf_angle_ave_z(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_angle_ave_z"))
        this%leaf_angle_sig_z(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_angle_sig_z"))
        this%leaf_length_ave(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_length_ave"))
        this%leaf_length_sig(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_length_sig"))
        this%leaf_width_ave(i)= freal(wheatconfig%parse(&
            config,key1="Leaf_"//str(i)//"_",key2="leaf_width_ave"))
        this%leaf_width_sig(i)= freal(wheatconfig%parse(&
...
!    this%mainroot_length = freal(wheatconfig%parse(config,key1="Mainroot",key2="Length"))
!    this%mainroot_width = freal(wheatconfig%parse(config,key1="Mainroot",key2="Width"))
!    this%mainroot_node = fint(wheatconfig%parse(config,key1="Mainroot",key2="Node"))
...
!    do 
!        line = wheatconfig%parse(config,key1="Branchroot_"//str(this%num_branch_root)//"_",key2="Node" )
!        if(len(trim(line))==0)then
...
!    enddo
    ...
    do i=1,this%num_leaf
        ...
        call this%leaf(num_leaf)%init(species=PF_wheat,&
    ...
    do
        ...
        line = wheatconfig%parse(config,key1="Panicle_"//str(this%num_panicle)//"_",key2="From")
...
! ########################################
subroutine mshwheat(this,name,num_threads)
    class(wheat_),intent(inout) :: this
...
! ########################################
subroutine vtkwheat(this,name,num_threads,single_file,&
...
! ########################################
subroutine jsonwheat(this,name)
    class(wheat_),intent(inout) :: this
    character(*),intent(in) :: name
...
! ########################################
subroutine stlwheat(this,name,num_threads,single_file)
    class(wheat_),intent(inout) :: this
    character(*),intent(in) :: name
    integer(int32),optional,intent(in) :: num_threads
    integer(int32) :: i,n
...
! ########################################
subroutine movewheat(this,x,y,z)
    class(wheat_),intent(inout) :: this
...
! ########################################
subroutine rotatewheat(this,x,y,z)
    class(wheat_),intent(inout) :: this
    real(real64),optional,intent(in) :: x,y,z
    integer(int32) :: i
...
! ########################################
recursive subroutine updatewheat(this,stem_id, root_id, leaf_id, overset_margin,debug)
    class(wheat_),intent(inout) :: this
...
!    endif
    ...
    ! if wheat_internode_info_ is active
    ! update parameters
...
!    endif
    ...
    if(.not. allocated(this%stem2stem ))then
        print *, "updatewheat >> ERROR :: .not. allocated(this%stem2stem )"
        return
    endif
    ...
    if(maxval(this%stem2stem)/=0) then
        ...
        do 
            ...
            if(present(debug) )then
                if(debug)then
                    print *, "wheat % update s2s >> error :: ",error
                endif
            endif
            if(itr > itr_tol) then
                print *, "wheat % update s2s >> ERROR :: not converged"
                stop
            endif
    ...
    do 
        ...
        if(present(debug) )then
            if(debug)then
                print *, "wheat % update r2s >> error :: ",error
            endif
        endif
        if(itr > itr_tol) then
            print *, "wheat % update r2s  >> ERROR :: not converged"
            stop
        endif
    ...
    do 
        ...
        if(present(debug) )then
            if(debug)then
                print *, "wheat % update r2r >> error :: ",error
            endif
        endif
        if(itr > itr_tol) then
            print *, "wheat % update r2r >> ERROR :: not converged"
            stop
        endif
    ...
    do 
        ...
        if(present(debug) )then
            if(debug)then
                print *, "wheat % update l2s >> error :: ",error
            endif
        endif
        if(itr > itr_tol) then
            print *, "wheat % update l2s  >> ERROR :: not converged"
            stop
        endif
...
! ########################################
!recursive subroutine updatewheat(this,debug)
!    class(wheat_),intent(inout) :: this
...
!        if(present(debug) )then
!            if(debug)then
!                print *, "wheat % update >> error :: ",error
!            endif
!        endif
!        if(itr > itr_tol) then
!            print *, "wheat % update >> ERROR :: not converged"
!            stop
!        endif
!        
...
subroutine removewheat(this,root)
    class(wheat_),intent(inout) :: this
...
! ################################################################
subroutine checkMemoryRequirementwheat(this)
    class(wheat_),intent(in) :: this
    real(real64) :: re_val
    integer(int64) :: val
...
! ##################################################################
function nswheat(this) result(ret)
    class(wheat_) ,intent(in) :: this
    integer(int32) :: ret, i
...
! ##################################################################
pure function nnwheat(this) result(ret)
    class(wheat_) ,intent(in) :: this
    integer(int32) :: ret, i
...
! ##################################################################
function newheat(this) result(ret)
    class(wheat_) ,intent(in) :: this
    integer(int32) :: ret, i
...
! ################################################################
recursive subroutine setYoungModuluswheat(this,YoungModulus,stem,root,leaf,panicle,ElementList)
    class(wheat_),intent(inout) :: this
...
! ################################################################
recursive subroutine setPoissonRatiowheat(this,PoissonRatio,stem,root,leaf,panicle)
    class(wheat_),intent(inout) :: this
    logical,optional,intent(in) :: stem, root, leaf, panicle
...
! ################################################################
recursive subroutine setDensitywheat(this,Density,stem,root,leaf,panicle)
    class(wheat_),intent(inout) :: this
    logical,optional,intent(in) :: stem, root, leaf, panicle
...
! ################################################################
function getEigenModewheat(this, ground_level,penalty,debug,Frequency,EbOM_Algorithm,num_mode) result(EigenVectors)
    class(wheat_),target,intent(inout) :: this
...
! ########################################
function numleafwheat(this) result(ret)
    class(wheat_),intent(in) :: this
    integer(int32) :: ret,i
...
! ########################################
function numStemwheat(this) result(ret)
    class(wheat_),intent(in) :: this
    integer(int32) :: ret,i
...
! ########################################
function numRootwheat(this) result(ret)
    class(wheat_),intent(in) :: this
    integer(int32) :: ret,i
...
! ########################################
function numPaniclewheat(this) result(ret)
    class(wheat_),intent(in) :: this
    integer(int32) :: ret,i
...
! ################################################################
function getFEMDomainPointerswheat(this) result(FEMDomainPointers)
    class(wheat_),target,intent(in) :: this
    type(FEMDomainp_),allocatable :: FEMDomainPointers(:)
...
! ################################################################
function checkYoungModuluswheat(this) result(all_young_modulus_is_set)
    class(wheat_),intent(in) :: this
    ...
    do i=1,this%numStem()
        if(.not.allocated(this%stem(i)%YoungModulus) )then
            ...
            print *, "[!Warning!] checkYoungModuluswheat >> Young Modulus is not set"
            print *, "@ Stem ID:",i
...
! ################################################################
function checkPoissonRatiowheat(this) result(all_young_modulus_is_set)
    class(wheat_),intent(in) :: this
    logical :: all_young_modulus_is_set 
    integer(int32) :: i
    ! order: stem -> leaf -> root
    ...
    do i=1,this%numStem()
        if(.not.allocated(this%stem(i)%PoissonRatio) )then
            all_young_modulus_is_set = .false.
            print *, "[!Warning!] checkPoissonRatiowheat >> Young Modulus is not set"
            print *, "@ Stem ID:",i
...
! ################################################################
function checkDensitywheat(this) result(all_young_modulus_is_set)
    class(wheat_),intent(in) :: this
    logical :: all_young_modulus_is_set 
    integer(int32) :: i
    ! order: stem -> leaf -> root
    ...
    do i=1,this%numStem()
        if(.not.allocated(this%stem(i)%Density) )then
            all_young_modulus_is_set = .false.
            print *, "[!Warning!] checkDensitywheat >> Young Modulus is not set"
            print *, "@ Stem ID:",i
...
function getYoungModulusFieldwheat(this) result(YoungModulus)
    class(wheat_),intent(inout) :: this
...
! ################################################################
function getPoissonRatioFieldwheat(this) result(PoissonRatio)
    class(wheat_),intent(inout) :: this
...
! ################################################################
function getDensityFieldwheat(this) result(Density)
    class(wheat_),intent(inout) :: this
...
function getYoungModuluswheat(this,DomainID,ElementID) result(YoungModulus)
    class(wheat_),intent(in) :: this
    ...
    if(DomainID > this%numStem() + this%numLeaf() + this%numRoot() &
        ...
        print *, "ERROR :: getYoungModuluswheat >>  DomainID exceeds max_domain_size"
        return
    endif
    ...
    elseif(this%numStem() + this%numLeaf() + this%numRoot() + 1 <= DomainID &
        ...
        print *, "[ERROR] >> getYoungModuluswheat >> Invalid DomainID",DomainID
        return
    endif
...
! ################################################################
function getPoissonRatiowheat(this,DomainID,ElementID) result(PoissonRatio)
    class(wheat_),intent(in) :: this
    ...
    if(DomainID > this%numStem() + this%numLeaf() + this%numRoot() &
        + this%numPanicle() )then
        print *, "ERROR :: getPoissonRatiowheat >>  DomainID exceeds max_domain_size"
        return
    endif
    ...
    elseif(this%numStem() + this%numLeaf() + this%numRoot() + 1 <= DomainID &
        ...
        print *, "[ERROR] >> getPoissonRatiowheat >> Invalid DomainID",DomainID
        return
    endif
...
! ################################################################
function getDensitywheat(this,DomainID,ElementID) result(Density)
    class(wheat_),intent(in) :: this
    integer(int32),intent(in) :: DomainID, ElementID
    real(real64) :: Density 
    integer(int32) :: i, n
    ...
    if(DomainID > this%numStem() + this%numLeaf() + this%numRoot() &
        +  this%numPanicle() )then
        print *, "ERROR :: getDensitywheat >>  DomainID exceeds max_domain_size"
        return
    endif
    ...
    elseif(this%numStem() + this%numLeaf() + this%numRoot() + 1 <= DomainID &
        ...
        print *, "[ERROR] >> getDensitywheat >> Invalid DomainID",DomainID
        return
    endif
...
subroutine deformwheat(this,displacement) 
    class(wheat_),target,intent(inout) :: this

    ! deform wheat by displacement
    ...
    if(present(displacement) )then
        if(size(displacement)/=this%nn()*3 )then
            print *, "ERROR :: deformwheat >> size(displacement) should be (this%numStem() + this%numLeaf() + this%numRoot())*3"
            return
        endif
...
function getElementListwheat(this,x_min,x_max,y_min,y_max,z_min,z_max,debug) result(ElementList)
    class(wheat_),intent(inout) :: this
    integer(int32),allocatable :: ElementList(:,:)
...
! ################################################################
function getDisplacementwheat(this, ground_level,penalty,debug,EbOM_Algorithm) result(displacement)
    class(wheat_),target,intent(inout) :: this
    real(real64),intent(in) :: ground_level
    real(real64),optional,intent(in) :: penalty
    logical,optional,intent(in) :: debug
...
! ################################################################
function getStressFieldwheat(this,displacement,i,j,option) result(StressField)
    class(wheat_),intent(inout) :: this
...
subroutine export_eigwheat(this,name,Frequency,ModeVectors,stress_type)
    class(wheat_),intent(inout) :: this
    character(*),intent(in) :: Name
