...
!        real(real64) :: K_o=165820.0d0 ! O2に対するミカエリス定数
   ...
   contains
      ...
      procedure, public :: change_length_or_width => change_length_or_width_Pod

      procedure, public :: rotate => rotatePod
      procedure, public :: move => movePod
      procedure, public :: curve => curvePod
      !procedure, public :: create => createPod

      procedure, pass :: connectPod => connectPod
      procedure, pass :: connectPodStem => connectPodStem

      generic :: connect => connectPod, connectPodStem

      procedure, public :: photosynthesis => photosynthesisPod
      procedure, public :: getPhotosynthesisSpeedPerVolume => getPhotosynthesisSpeedPerVolumePod

      procedure, public :: rescale => rescalePod
      procedure, public :: adjust => adjustPod
      procedure, public :: resize => resizePod

      ! check condition
      ! is it empty?
      procedure, public :: empty => emptyPod
      procedure, public :: getVolume => getVolumePod
      procedure, public :: getLength => getLengthPod
      procedure, public :: getBiomass => getBiomassPod
      procedure, public :: getCoordinate => getCoordinatePod
      procedure, public :: getPodArea => getPodAreaPod
      procedure, public :: getRadius => getRadiusPod
      procedure, public :: getCenter => getCenterPod
      procedure, public :: getNormalVector => getNormalVectorPod

      procedure, public :: FullyExpanded => FullyExpandedPod

      procedure, public :: gmsh => gmshPod
      procedure, public :: msh => mshPod
      procedure, public :: vtk => vtkPod
      procedure, public :: stl => stlPod
      procedure, public :: ply => plyPod

      procedure, public :: sync => syncPod

      procedure, public :: nn => nnPod
      procedure, public :: ne => nePod

      ! remove
      procedure, public :: remove => removePod
   end type

   interface operator(//)
      module procedure append_Pod_object_vector
   end interface
contains

!   subroutine createPod(obj, SurfacePoints, filename, x_num, y_num, x_len, y_len)
...
!
!      else
!         print *, "ERROR :: Pod%create >> Please import SurfacePoints or Filename"
!         stop
!      end if
!
...
! ########################################
   subroutine initPod(obj, config, regacy, Thickness, length, width, ShapeFactor, &
                       MaxThickness, Maxlength, Maxwidth, rotx, roty, rotz, location, species, SoyWidthRatio, &
                       curvature, x_num, y_num, z_num,shape_parameters)
      class(Pod_), intent(inout) :: obj
      real(real64), optional, intent(in) :: Thickness, length, width, ShapeFactor,shape_parameters(:)
      ...
      real(real64) :: loc(3), radius, z, Pod_L,r1,r2,l,petiole_ratio,theta
      real(real64),allocatable :: shape_params(:)
      logical :: debug = .false.
...
!            ! デフォルトの設定を生成
!            if(debug) print *, "New Pod-configuration >> Podconfig.json"
!            call Podconf%open("Podconfig.json")
!            write(Podconf%fh,*) '{'
!            write(Podconf%fh,*) '   "type": "Pod",'
!            write(Podconf%fh,*) '   "minlength": 0.005,'
!            write(Podconf%fh,*) '   "minwidth": 0.005,'
!            write(Podconf%fh,*) '   "minthickness": 0.0001,'
!            write(Podconf%fh,*) '   "maxlength": 0.07,'
!            write(Podconf%fh,*) '   "maxwidth": 0.045,'
!            write(Podconf%fh,*) '   "maxthickness": 0.001,'
!            write(Podconf%fh,*) '   "shaperatio": 0.3,'
!            write(Podconf%fh,*) '   "drydensity": 0.0,'
!            write(Podconf%fh,*) '   "watercontent": 0.0,'
!            write(Podconf%fh,*) '   "xnum": 10,'
!            write(Podconf%fh,*) '   "ynum": 10,'
!            write(Podconf%fh,*) '   "znum": 20'
!            write(Podconf%fh,*) '}'
!            conf="Podconfig.json"
!            call Podconf%close()
!        else
...
!         conf = config
!         call Podconf%open(conf)
!         blcount = 0
!         do
!            read (Podconf%fh, '(a)') line
!            if (debug) print *, line
...
!            if (blcount == 1) then
!
!               if (index(line, "type") /= 0 .and. index(line, "Pod") == 0) then
!                  print *, "ERROR: This config-file is not for Pod"
!                  return
!               end if
!
...
!
!      ! メッシュを生成
!      call obj%FEMdomain%create(meshtype="Cylinder3D", x_num=obj%xnum, y_num=obj%ynum, z_num=obj%znum, &
...
!      if (present(config)) then
!         obj%DryDensity(:) = freal(Podconf%parse(config, key1="drydensity"))
!         obj%watercontent(:) = freal(Podconf%parse(config, key1="watercontent"))
!      end if
...
!      call obj%FEMdomain%remove()
      ...
      if (present(species)) then
         if (species == PF_SOYBEAN_CV) then
            call obj%FEMdomain%create(meshtype="Sphere3D", x_num=obj%xnum, y_num=obj%ynum, z_num=obj%znum, &
         ...
         elseif(species == PF_Arabidopsis)then
            ...
            !call obj%FEMDomain%rotate(x=radian(90.0d0),y=radian(90.0d0))
               call obj%FEMDomain%move(&
                  x=-(obj%FEMDomain%xmax() + obj%FEMDomain%xmin())/2.0d0,&
                  y=-(obj%FEMDomain%ymax() + obj%FEMDomain%ymin())/2.0d0,&
                  z=-(obj%FEMDomain%zmin()))
                  ! pod-like modification
               l  = obj%FEMDomain%zmax()
               
               if(present(shape_parameters))then
                  shape_params = shape_parameters 
               else
                  ! length ratio of (1)petiole, (3) pod, and transition area (2,4), and (5) r_petiole/r_pod
                  shape_params = [0.250d0, 0.10d0, 0.55d0, 0.10d0, 0.50d0]
               endif
               !shape_params(1:4) = shape_params(1:4)/sum(shape_params(1:4))

               
               do i=1,obj%femdomain%nn()
                  z = obj%femdomain%mesh%nodcoord(i,3)
                  if(z < shape_params(1)*l )then
                     ! petiole
                     obj%femdomain%mesh%nodcoord(i,1:2) = shape_params(5)*obj%femdomain%mesh%nodcoord(i,1:2)
                  elseif(z < sum(shape_params(1:2))*l )then
                     ! transition area
                     theta = (z-shape_params(1)*l)/(shape_params(2)*l)
                     obj%femdomain%mesh%nodcoord(i,1:2) = &
                        (shape_params(5)*(1.0d0-theta) + 1.0d0*theta)*obj%femdomain%mesh%nodcoord(i,1:2)
                  elseif(z < sum(shape_params(1:3))*l )then
                     ! pod area
                     ! do nothing
                     obj%femdomain%mesh%nodcoord(i,1:2) = obj%femdomain%mesh%nodcoord(i,1:2)
                  else
                     theta = (z-sum(shape_params(1:3))*l)/(shape_params(4)*l)
                     obj%femdomain%mesh%nodcoord(i,1:2) = &
                        (shape_params(5)*theta + 1.0d0*(1.0d0-theta))*obj%femdomain%mesh%nodcoord(i,1:2)
                  endif
               enddo
            ...
            call obj%FEMdomain%create(meshtype="Pod3D", x_num=obj%xnum, y_num=obj%ynum, z_num=obj%znum, &
...
! ########################################
   ...
   subroutine curvePod(obj, curvature)
      ! deform by curvature
      class(Pod_), intent(inout) :: obj
      real(real64), intent(in) :: curvature
      real(real64) :: Pod_L, radius, z
      integer(int32) :: i

      if (curvature < dble(1.0e-5)) then
         print *, "Caution >> initPod >> curvature is too small < 1.0e-5"
         print *, "Then, ignored."
         return
      end if
      radius = 1.0d0/curvature
      Pod_L = maxval(obj%femdomain%mesh%nodcoord(:, 3)) - minval(obj%femdomain%mesh%nodcoord(:, 3))
      Pod_L = 0.50d0*Pod_L
      do i = 1, obj%femdomain%nn()
         z = obj%femdomain%mesh%nodcoord(i, 3)
         obj%femdomain%mesh%nodcoord(i, 2) = &
            obj%femdomain%mesh%nodcoord(i, 2) &
            - sqrt(radius*radius - Pod_L*Pod_L) &
            + sqrt(radius*radius - (z - Pod_L)*(z - Pod_L))
      end do

   end subroutine

! ########################################
   recursive subroutine rotatePod(obj, x, y, z, reset)
      class(Pod_), intent(inout) :: obj
...
! ########################################
   recursive subroutine movePod(obj, x, y, z, reset)
      class(Pod_), intent(inout) :: obj
      real(real64), optional, intent(in) :: x, y, z
      logical, optional, intent(in) :: reset
      real(real64), allocatable :: origin1(:), origin2(:), disp(:)
...
! ########################################
   subroutine connectPod(obj, direct, Pod)
      class(Pod_), intent(inout) :: obj
      class(Pod_), intent(inout) :: Pod
      ...
      !        x1 = Pod%getCoordinate("A")
      !        x2 = stem%getCoordinate("B")
      !        disp = x2 - x1
      !        call Pod%move(x=disp(1),y=disp(2),z=disp(3) )
      !    endif
!
!
      !    if(direct=="<-" .or. direct=="<=")then
      ...
      if (direct == "->" .or. direct == "=>") then
         ! move obj to connect Pod (Pod is not moved.)
...
! ########################################
   subroutine connectPodStem(obj, direct, Stem)
      class(Pod_), intent(inout) :: obj
...
! ########################################
   function getCoordinatePod(obj, nodetype) result(ret)
      class(Pod_), intent(inout) :: obj
...
! ########################################
   subroutine vtkPod(obj, name, field_name)
      class(Pod_), intent(inout) :: obj
      character(*), intent(in) ::name
...
! ########################################
   subroutine resizePod(obj, x, y, z, Length, Width)
      class(Pod_), optional, intent(inout) :: obj
...
! ########################################
   function getLengthPod(obj) result(Length)
      class(Pod_), optional, intent(inout) :: obj
      real(real64) :: Length
      real(real64), allocatable :: origin1(:), origin2(:), disp(:), total_rot(:)
...
! ########################################
   function FullyExpandedPod(obj, threshold) result(ret_expanded)
      class(Pod_), optional, intent(inout) :: obj
...
! ########################################
   subroutine rescalePod(obj, x, y, z)
      class(Pod_), optional, intent(inout) :: obj
      real(real64), optional, intent(in) :: x, y, z
      real(real64), allocatable :: origin1(:), origin2(:), disp(:)
...
! ########################################
!subroutine LayTracingPod(obj,maxPPFD,light,)
!    class(Pod_),intent(inout) :: obj
...
! ########################################
   subroutine photosynthesisPod(obj, dt, air)
...
! ########################################
   function getPhotosynthesisSpeedPerVolumePod(obj, dt, air) result(Speed_PV)
...
! ####################################################################
   subroutine adjustPod(obj, width)
      class(Pod_), intent(inout) :: obj
      real(real64), intent(in) :: width(:, :)

   end subroutine

   function getVolumePod(obj) result(ret)
      class(Pod_), intent(in) :: obj
      real(real64) :: ret
      integer(int32) :: i, j
...
! ########################################
   function getBiomassPod(obj) result(ret)
      class(Pod_), intent(in) :: obj
      real(real64) :: ret
      integer(int32) :: i, j
...
! ########################################
   function emptyPod(obj) result(Pod_is_empty)
      class(Pod_), intent(in) :: obj
      logical :: Pod_is_empty

      Pod_is_empty = obj%femdomain%empty()

   end function
! ########################################

   function getPodAreaPod(obj) result(PodArea)
      class(Pod_), intent(in) :: obj
      real(real64) :: PodArea

      PodArea = obj%getVolume()/obj%thickness

   end function
! ########################################

! ################################################################
   pure function getRadiusPod(obj) result(radius)
      class(Pod_), intent(in) :: obj
...
! ################################################################
   pure function getCenterPod(obj) result(Center)
      class(Pod_), intent(in) :: obj
      real(real64), allocatable :: Points(:, :)
...
! ################################################################
   ...
   subroutine syncPodVector(obj, from, mpid)
      type(Pod_), allocatable, intent(inout) :: obj(:)
      integer(int32), intent(in) :: from
      type(MPI_), intent(inout) :: mpid
...
! ##################################################################
   function getNormalVectorPod(obj, ElementID) result(ret)
      class(Pod_), intent(inout) :: obj
      ...
      if (num_effective_norms == 0) then
         print *, "ERROR :: getNormalVectorPod >> no valid normal vector is found"
         stop
      end if
...
! #################################################################
   subroutine change_length_or_width_Pod(this, dt)
      class(Pod_), intent(inout)::this
      real(real64), intent(in) :: dt
...
! ###############################################################
   subroutine removePod(this)
      class(Pod_), intent(inout) :: this

      call this%FEMDomain%remove()
      if (allocated(this%PodSurfaceNode2D)) deallocate (this%PodSurfaceNode2D)
      ...
      this%Division = 0
      if (associated(this%pPod)) nullify (this%pPod)
   ...
   function nnPod(this) result(ret)
      class(Pod_), intent(in) :: this
      integer(int32) :: ret

      ret = this%femdomain%nn()
   end function

   function nePod(this) result(ret)
      class(Pod_), intent(in) :: this
      integer(int32) :: ret
...
! ############################################################
   function append_Pod_object_vector(arg1, arg2) result(ret)
      type(Pod_), allocatable, intent(in) :: arg1(:), arg2(:)
      type(Pod_), allocatable :: ret(:)
...
!      end if
!
!      print *, "Caution :: no input is in initPod"
!
!   end subroutine
!! ########################################
!
