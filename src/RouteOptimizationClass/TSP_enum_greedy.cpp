...
{
	min=0.;
	//for(int i=0; i<Node; i++) route[i]=i;//スタートを固定
	random_order(route);//並びがランダムの場合
	...
	for(int i=1; i<Node; i++)//点を固定していく、現在の点がi
	{
		...
		{
			if(dist>distance[route[i-1]][route[j]])
			{
				dist=distance[route[i-1]][route[j]];
				number=j;
				count+=1;
			}
		}
...
void Points::random_order(vector<int> &route)
{
	//シードで乱数を使いたい場合//
	//int seed=65;
	//mt19937 mt(seed);
